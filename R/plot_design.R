#' Plotting S3 method for objects of class `design`
#'
...
plot.design = function(x, ...) {
  ...
  # Arguments
  arguments = list(...)
  main_text = ifelse(is.null(arguments[["main"]]),
                     paste0("Flowchart of '", object_argument_name, "'"),
                     arguments[["main"]])
  ...
  # Lane labels
  lane_map = list("dgp" = 1, "estimand" = 2, "estimator" = 3)
  grid.text("DGP",
            x = label_offset,
  ...
  for(i in seq.int(length(x))) {
    ...
    current_node = list(lane = attr(x[[i]], "causal_type"),
    ...
    if(current_node[["step"]] == "wrapped" &&
      ...
      current_node[["label"]] = tryCatch({
        paste0(
          as.character(node_car(get_expr(attr(x[[i]], "call"))))[c(2, 1, 3)],
          collapse = "")
      }, error = function(e) {
        return("Unknown")
      })
    ...
    current_node[["x"]] = node_offset + ((node_width + node_interstitial) *
                                           (global_index -
                                              global_index_modifier - 1))

    flow_box(current_node[["x"]], current_node[["y"]],
    ...
    if(!is.null(old_node)) {
      if(current_node[["lane"]] == "dgp") {
        straight_line(old_node[["x"]] + (node_width / 2),
                      old_node[["y"]],
                      current_node[["x"]] - (node_width / 2),
                      current_node[["y"]])
      } else {
        curvy_line(old_node[["x"]] + (node_width / 2),
                   old_node[["y"]],
                   current_node[["x"]] - (node_width / 2),
                   current_node[["y"]])
      }
...
#' Maps a node into a grid label
#'
#' Convert node step to real text, then append label if it exists
#' @param node a single design_step object from a design object
#' @return a character vector
#' @keywords internal
process_node_label = function(node) {
  base_name = map_plot_box_name(node[["step"]])
  name = ifelse(is.null(node[["label"]]) || nchar(node[["label"]]) == 0,
                base_name,
                paste0(base_name, ":\n", node[["label"]]))
}

#' Maps a step name into a plot-friendly English name
#'
#' Replaces underscores with newlines, or manually maps nodes we wish to rename.
#'
#' @param name a character vector from a step attribute of a design_step object.
#' @return a character vector
...
map_plot_box_name = function(name) {
  ...
  if(name %in% names(name_map)) { return(unlist(unname(name_map[name]))) }
  return(toTitleCase(gsub("_", "\n", name)))
}

#' Trims white space in each line of text within a character vector
#'
...
#' Draws a rounded rectangle box with a label
#'
#' @param x x coordinate in the initial 0-1 viewport
#' @param y y coordinate in the initial 0-1 viewport
#' @param text A character vector label for the node
#' @param ... Graphical parameters, currently supported `fill`, `boxwidth`,
#' `boxheight`, `leftmargin`, `text.cex`.
#' @importFrom grid unit viewport pushViewport grid.roundrect
#' @importFrom grid gpar grid.text popViewport
#' @keywords internal
flow_box <- function(x, y, text, ...) {
  ...
  box_vp <- viewport(x = x, y = y, width = box_width, height = box_height)
  pushViewport(box_vp)
  grid.roundrect(gp=gpar(fill=fill_color))
  grid.text(text, x = text_left_margin,
...
#'
#' @param x1 x coordinate in the initial 0-1 viewport
#' @param y1 y coordinate in the initial 0-1 viewport
#' @param x2 x coordinate in the initial 0-1 viewport
#' @param y2 y coordinate in the initial 0-1 viewport
#' @param ... Graphical parameters, currently supported `arrow_size`
#' and `line_type`
...
#' Draws a curvy, and by default dashed, line.
#'
#' @param x1 x coordinate in the initial 0-1 viewport
#' @param y1 y coordinate in the initial 0-1 viewport
#' @param x2 x coordinate in the initial 0-1 viewport
